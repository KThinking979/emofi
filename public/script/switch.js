class Switch {
  constructor(world, x, y, color, face) {
    ...
    let path = face.includes('S')
      ? `images/snakes/${face}.png`
      : `images/anys/${face}.png`;
      ...
      event.pairs.forEach(pair => {
        if (pair.bodyA === this.body || pair.bodyB === this.body) {
          ...
          if (bodyCollide.label.includes('FOOD')) {
            if (color !== this.color && !this.isDie) {
              this.die(true);
            }
          }
        }
  ...
  die(isCheck) {
    ...
    if (scores > bestGoalSwitch) {
      bestGoalSwitch = scores;
    }

    window.ReactNativeWebView.postMessage(
      `${JSON.stringify({ dieSwitch: scores })}`,
    );
  }
  ...
  upPosition() {
    this.body.position.y > 0 &&
      Matter.Body.setVelocity(this.body, { x: 0, y: this.isFirst ? -10 : -5 });
    if (this.isFirst) {
      this.isFirst = false;
    }
     ...
     * Skill 2
     */
    if (this.body.position.y > 0 && runSkill_2) {
      Matter.Body.setVelocity(this.bodySkill2A, { x: 0, y: -5 });
      Matter.Body.setVelocity(this.bodySkill2B, { x: 0, y: -5 });
      Matter.Body.setVelocity(this.bodySkill2C, { x: 0, y: -5 });
      Matter.Body.setVelocity(this.bodySkill2D, { x: 0, y: -5 });
      // Matter.Body.setPosition(this.bodySkill2A, {
      //   x: this.bodySkill2A.position.x,
      //   y: this.bodySkill2A.position.y - 5,
      // })

      // Matter.Body.setPosition(this.bodySkill2B, {
      //   x: this.bodySkill2B.position.x,
      //   y: this.bodySkill2B.position.y - 5,
      // })

      // Matter.Body.setPosition(this.bodySkill2C, {
      //   x: this.bodySkill2C.position.x,
      //   y: this.bodySkill2C.position.y - 5,
      // })

      // Matter.Body.setPosition(this.bodySkill2D, {
      //   x: this.bodySkill2D.position.x,
      //   y: this.bodySkill2D.position.y - 5,
      // })
    }
  }

  update() {
    if (this.body.position.y <= h / 2) {
      isNext = true;
    }
    ...
    if (!this.isDie && this.body.position.y >= h + this.sizeEmoji) {
      this.die();
    }
